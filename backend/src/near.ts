...
const MpcContractId = isProxyCall ? NEAR_PROXY_ACCOUNT_ID : MPC_CONTRACT_ID;
const privateKey =
...
console.log('NCS contractId', MpcContractId);
...
export async function sign(payload: any, path: string) {
    ...
    let attachedDeposit = nearAPI.utils.format.parseNearAmount('0.5');
...
export async function getChallengeById(id: string) {
    const res = await account.viewFunction({
        contractId: NEAR_CHALLENGE_CONTRACT_ID,
        methodName: 'get_challenge_by_id',
        args: { "id" : id },
    });

    console.log("challegen data: " , res);
    return res;
}