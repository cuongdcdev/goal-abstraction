...
// Challenge class
class Challenge {
  constructor(id, title, description, expire, chain, prize, judger, success_addr, failed_addr, status) {
    this.id = id;
    this.title = title;
    ...
    this.prize = prize;
    this.judger = judger;
    this.success_addr = success_addr;
    this.failed_addr = failed_addr;
    this.status = status;
  }
}
let ChallengeContract = (_dec = NearBindgen({}), _dec2 = initialize(), _dec3 = call({
  payableFunction: true
}), _dec4 = call({}), _dec5 = view(), _dec6 = view(), _dec7 = call({}), _dec(_class = (_class2 = class ChallengeContract {
  constructor() {
  ...
  init({
    ...
    this.owner = owner || currentAccountId();
  }
  /**
  ...
  create({
    ...
    if (chain == "NEAR") {
      let attached_deposit = attachedDeposit();
    ...
    assert(expire > blockTimestamp(), "Expired time must be greater than current block time, current block timestamp: " + blockTimestamp());
    const randomSeed$1 = randomSeed();
    const id = `id${Array.from(randomSeed$1).map(b => b.toString(16).padStart(2, '0')).join('')}`;
    const challenge = new Challenge(id, title, description, expire, chain, prize, judger, success_addr, failed_addr, Status.Open);
    this.challenges.set(id, challenge);

    // Emit event
    log(`EVENT_JSON:{"standard":"goforit","version":"1.0.0","event":"challenge_created","data":${JSON.stringify({
      id: challenge.id,
  ...
  finalize({
    ...
    if (predecessorAccountId() !== challenge.judger) {
    ...
    if (blockTimestamp() < challenge.expire) {
      throw new Error('Challenge has not expired yet, the expired time is around: ' + challenge.expire + ' current block timestamp: ' + blockTimestamp());
    }
    ...
    if (challenge.chain === 'NEAR') {
      ...
      const promise = promiseBatchCreate(targetAddress);
      promiseBatchActionTransfer(promise, remainingPrize);
    }
    ...
    // Emit event
    log(`EVENT_JSON:{"standard":"goforit","version":"1.0.0","event":"challenge_updated","data":${JSON.stringify({
      id: challenge.id,
  ...
  // }
}, _applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "create", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "create"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "finalize", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "finalize"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_all_challenges", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "get_all_challenges"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_challenge_by_id", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "get_challenge_by_id"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "set_service_fee", [_dec7], Object.getOwnPropertyDescriptor(_class2.prototype, "set_service_fee"), _class2.prototype), _class2)) || _class);
function set_service_fee() {
  const _state = ChallengeContract._getState();
  if (!_state && ChallengeContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = ChallengeContract._create();
  if (_state) {
    ChallengeContract._reconstruct(_contract, _state);
  }
  const _args = ChallengeContract._getArgs();
  const _result = _contract.set_service_fee(_args);
  ChallengeContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(ChallengeContract._serialize(_result, true));
}
function get_challenge_by_id() {
  const _state = ChallengeContract._getState();
  if (!_state && ChallengeContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = ChallengeContract._getArgs();
  const _result = _contract.get_challenge_by_id(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(ChallengeContract._serialize(_result, true));
}
function get_all_challenges() {
  ...
  const _args = ChallengeContract._getArgs();
  const _result = _contract.get_all_challenges(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(ChallengeContract._serialize(_result, true));
}
function finalize() {
  const _state = ChallengeContract._getState();
  ...
  const _args = ChallengeContract._getArgs();
  const _result = _contract.finalize(_args);
  ChallengeContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(ChallengeContract._serialize(_result, true));
}
function create() {
  ...
  const _args = ChallengeContract._getArgs();
  const _result = _contract.create(_args);
  ChallengeContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(ChallengeContract._serialize(_result, true));
}
function init() {
...
export { create, finalize, get_all_challenges, get_challenge_by_id, init, set_service_fee };
//# sourceMappingURL=goforit.js.map
